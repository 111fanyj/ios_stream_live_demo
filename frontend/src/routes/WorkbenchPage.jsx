...
function formatTargetSummary(target) {
  ...
  if (target.ref) {
    ...
    segments.push(
      `(${Number(target.x).toFixed(0)}, ${Number(target.y).toFixed(0)}) px`,
    )
  }
  ...
  if (Number.isFinite(offsetX) || Number.isFinite(offsetY)) {
    segments.push(
      `偏移 (${Number.isFinite(offsetX) ? offsetX.toFixed(0) : '0'}, ${Number.isFinite(offsetY) ? offsetY.toFixed(0) : '0'}) px`,
    )
  }
...
function formatStepSummary(step) {
  ...
  if (step.type === 'waitForText' || step.type === 'loopUntilText') {
    return [step.query, ...(step.queryOptions || [])]
      .filter(Boolean)
      .join(' / ')
  }
...
function buildStepDetailLines(step) {
  ...
  if (isTextWaitType(step.type)) {
    ...
    lines.push(
      `匹配: ${step.match || 'contains'} / 超时 ${Number(step.timeoutMs || 10000)} ms / 轮询 ${Number(step.pollIntervalMs || 500)} ms`,
    )
    if (step.saveAs) {
  ...
  if (isImageWaitType(step.type)) {
    lines.push(
      `模板: ${step.assetId || '未配置'} / 阈值 ${Number(step.threshold ?? 0.84).toFixed(2)}`,
    )
    lines.push(
      `超时 ${Number(step.timeoutMs || 10000)} ms / 轮询 ${Number(step.pollIntervalMs || 500)} ms`,
    )
    if (step.saveAs) {
      lines.push(`保存命中点: ${step.saveAs}`)
    }
  ...
  if (isLoopType(step.type) && step.action) {
    lines.push(
      `loop 动作: ${STEP_TYPE_LABELS[step.action.type] || step.action.type}`,
    )
    ...
    if (step.action.type === 'drag') {
      ...
      lines.push(
        `loop hold ${Number(step.action.holdMs || 120)} ms / drag ${Number(step.action.durationMs || 450)} ms`,
      )
    }
  }
...
function buildStepFromForm(stepForm, currentStepCount) {
  ...
  const id =
    String(stepForm.id || '').trim() || `${type}-${currentStepCount + 1}`
  ...
  if (isTextWaitType(type)) {
    ...
    if (isLoopType(type)) {
      if (stepForm.loopActionType === 'drag') {
        const from = createTarget(
          stepForm.fromRef,
          stepForm.fromX,
          stepForm.fromY,
          stepForm.fromOffsetX,
          stepForm.fromOffsetY,
        )
        const to = createTarget(
          stepForm.toRef,
          stepForm.toX,
          stepForm.toY,
          stepForm.toOffsetX,
        ...
        const target = createTarget(
          ...
          stepForm.targetX,
          stepForm.targetY,
          stepForm.targetOffsetX,
          stepForm.targetOffsetY,
        )
        if (!target) {
...
function buildAutomationDocument(editorDraft) {
  return {
    ...
    revision: 0,
    name:
      String(editorDraft.name || '').trim() ||
      String(editorDraft.packageId || '').trim() ||
      'Demo Flow',
...
function clampCrop(nextCrop) {
  ...
  const width = Math.max(
    minSize,
    Math.min(1, Number(nextCrop.width) || minSize),
  )
  const height = Math.max(
    minSize,
    Math.min(1, Number(nextCrop.height) || minSize),
  )
...
function cropFromPointerDrag(drag, event) {
  ...
  const dx =
    (event.clientX - drag.startClientX) / Math.max(1, drag.shellRect.width)
  const dy =
    (event.clientY - drag.startClientY) / Math.max(1, drag.shellRect.height)
...
export function WorkbenchPage() {
  ...
  const {
    ...
    error: healthError,
  } = useHealth(viewer.normalizedServerUrl)
  const {
    packages,
  ...
  const [editorStatus, setEditorStatus] = useState(
    '可从右侧选中 revision 载入，也可以直接新建流程',
  )
  const [publishStatus, setPublishStatus] = useState(
    '编辑完成后可发布为新的 revision',
  )
  ...
  const selectedPackage = useMemo(
    () =>
      packages.find(
        (entry) => entry.packageId === executionSelection.packageId,
      ) || null,
  ...
  const selectedRevisions = useMemo(() => {
    ...
    return [...selectedPackage.revisions].sort(
  ...
  useEffect(() => {
    ...
    setExecutionSelection((current) => {
      const existingPackage = packages.find(
        (entry) => entry.packageId === current.packageId,
      )
      if (existingPackage) {
        return current
      }
  ...
  useEffect(() => {
    ...
    const preferredRevision =
      ...
      selectedRevisions.some(
        (entry) => String(entry.revision) === executionSelection.revision,
      )
        ? executionSelection.revision
  ...
  const overlayElements = useMemo(
    () =>
      viewer.overlayItems.flatMap((item, index) => {
        if (item.kind === 'point') {
          ...
          return [
            <div
              ...
              style={{
                left: `${clamp01(point.x) * 100}%`,
                top: `${clamp01(point.y) * 100}%`,
              }}
            />,
            <div
              key={`label-${index}`}
        ...
        if (
          ...
          return [
            ...
            <div
              ...
              className="overlay-marker-react"
              style={{
                left: `${clamp01(item.from.x) * 100}%`,
                top: `${clamp01(item.from.y) * 100}%`,
              }}
            />,
            <div
            ...
            <div
              ...
              className="overlay-marker-react"
              style={{
                left: `${clamp01(item.to.x) * 100}%`,
                top: `${clamp01(item.to.y) * 100}%`,
              }}
            />,
            <div
              key={`to-label-${index}`}
  ...
  function handleStartExecution() {
    const result = viewer.startAutomation(
      executionSelection.packageId,
      Number(executionSelection.revision),
    )
    setExecutionMessage(
      result.ok
  ...
  function handleStopExecution() {
    ...
    setExecutionMessage(
      result.ok ? '已发送停止执行' : result.error || '发送停止执行失败',
    )
  }
  ...
  function handleSelectPackageCard(entry) {
    setExecutionSelection({
      ...
      revision: getPreferredRevision(entry),
    })
    setExecutionMessage(`已切换到 ${entry.packageId}`)
  }

  function handleAddStep() {
    const { step, error } = buildStepFromForm(
      stepForm,
  ...
  async function handleAssetFileChange(event) {
    ...
    try {
      ...
      const assetId = createNextAssetId(
        editorDraft.images,
        assetDraftId || stepForm.assetId || file.name.replace(/\.[^.]+$/, ''),
      )

      setEditorDraft((current) => ({
        ...current,
        images: [
          ...current.images.filter((asset) => asset.assetId !== assetId),
          { assetId, dataUrl: pngDataUrl },
        ],
      }))
  ...
  function handleCaptureAssetFromVideo() {
    ...
    const assetId = createNextAssetId(
      editorDraft.images,
      assetDraftId || stepForm.assetId || 'capture',
    )
  ...
  async function handlePublishPackage() {
    ...
    if (!isValidPackageId(automation.packageId)) {
      setPublishStatus(
        'Package ID 只能包含字母、数字、下划线或连字符，长度 1-64',
      )
      return
    }
    ...
    try {
      ...
      setPublishDownloadUrl(
        revision.downloadUrl
          ? buildHttpUrl(viewer.normalizedServerUrl, revision.downloadUrl)
          : '',
      )
  ...
  return (
    <div className="page-grid page-grid-workbench">
      <section className="panel hero-panel">
        <div>
          ...
          <p className="panel-copy">
            React 工作台已经接上 viewer 信令、WebRTC
            视频、overlay、模板截屏、方案执行、revision
            明细和发布流程。当前仍保留 legacy 页面作为对照入口。
          </p>
        </div>
        <div className="status-row">
          ...
          <span className="status-chip">
            健康接口:{' '}
            {healthLoading ? '加载中' : healthError ? '失败' : '已连接'}
          </span>
          <span className="status-chip">
            方案接口:{' '}
            {packagesLoading
      ...
      <section className="workbench-top-grid">
        <details
          className="panel collapsible-panel connection-panel-react"
          open
        >
        ...
        <details className="panel collapsible-panel log-panel-react">
          ...
          <div className="collapsible-body stack-panel">
            <pre className="console-panel">
              {viewer.debugLines.length > 0
                ? viewer.debugLines.join('\n')
                : '等待连接...'}
            </pre>
          </div>
        </details>
          ...
          <div className="collapsible-body stack-panel">
            <div className="button-row">
              <button
                type="button"
                className="button-secondary"
                onClick={viewer.clearOverlay}
                disabled={viewer.overlayItems.length === 0}
              >
                清空 overlay
              </button>
            </div>
            ...
            <pre className="console-panel">
              {viewer.automationLines.length > 0
                ? viewer.automationLines.join('\n')
                : '等待自动化事件...'}
            </pre>
          </div>
        </details>
        ...
        <section className="panel viewer-panel-react workspace-frame-panel">
          <div className="panel-header">
            ...
            <div className="status-row compact-status-row-react">
              <span className={viewer.isLive ? 'badge ok' : 'badge'}>
                {viewer.isLive ? '直播中' : '等待直播'}
              </span>
              <span className="badge">
                Overlay {viewer.overlayItems.length}
              </span>
            </div>
          </div>
          <div className="viewer-stage-react">
            <div
              ...
              className={
                viewer.remoteStream
                  ? 'video-shell-react live'
                  : 'video-shell-react empty'
              }
            >
              ...
              {!viewer.remoteStream ? (
                <div className="video-empty-state">
                  ...
                  <p>
                    先连接同一个 room，收到 offer 后这里会自动建立视频轨道。
                  </p>
                </div>
              ) : null}
            </div>
          ...
          <div className="editor-layout-react">
            <section className="nested-panel-react">
              ...
              <div className="form-grid-react compact-grid-react">
                <label className="field-label">
                  ...
                  <input
                    value={editorDraft.packageId}
                    onChange={(event) =>
                      handleUpdateEditorField('packageId', event.target.value)
                    }
                    placeholder="例如 demo"
                  />
                </label>
                <label className="field-label">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      handleUpdateEditorField('name', event.target.value)
                    }
                    placeholder="例如 Demo Flow"
                  />
                </label>
              </div>
              <div className="panel-header">
                ...
                <>
                  {isTextWaitType(stepForm.type) ? (
                    <label className="field-label">
                      <span>文字查询</span>
                      <textarea
                        value={stepForm.queryText}
                        onChange={(event) =>
                          handleUpdateStepField('queryText', event.target.value)
                        }
                        rows="4"
                        placeholder={'每行一个候选文字，例如\n登录\n立即开始'}
                      />
                    </label>
                  ) : null}
                  ...
                  <div className="form-grid-react compact-grid-react">
                    <label className="field-label">
                      <span>保存为</span>
                      <input
                        value={stepForm.saveAs}
                        onChange={(event) =>
                          handleUpdateStepField('saveAs', event.target.value)
                        }
                ...
                <div className="subtle-panel-react">
                  ...
                  <div className="form-grid-react compact-grid-react">
                    <label className="field-label">
                      <span>x 像素</span>
                      <input
                        value={stepForm.targetX}
                        type="number"
                        onChange={(event) =>
                          handleUpdateStepField('targetX', event.target.value)
                        }
                        placeholder="例如 320"
                      />
                    </label>
                    <label className="field-label">
                      <span>y 像素</span>
                      <input
                        value={stepForm.targetY}
                        type="number"
                        onChange={(event) =>
                          handleUpdateStepField('targetY', event.target.value)
                        }
                        placeholder="例如 480"
                      />
                    </label>
                ...
                <div className="subtle-panel-react">
                  ...
                  <div className="form-grid-react compact-grid-react">
                    <label className="field-label">
                      <span>起点引用</span>
                      <input
                        value={stepForm.fromRef}
                        onChange={(event) =>
                          handleUpdateStepField('fromRef', event.target.value)
                        }
                        placeholder="可选"
                      />
                    </label>
                    <label className="field-label">
                      <span>终点引用</span>
                      <input
                        value={stepForm.toRef}
                        onChange={(event) =>
                          handleUpdateStepField('toRef', event.target.value)
                        }
                        placeholder="可选"
                      />
                    </label>
                  ...
                  <div className="form-grid-react compact-grid-react">
                    <label className="field-label">
                      <span>起点 x</span>
                      <input
                        value={stepForm.fromX}
                        type="number"
                        onChange={(event) =>
                          handleUpdateStepField('fromX', event.target.value)
                        }
                        placeholder="例如 320"
                      />
                    </label>
                    <label className="field-label">
                      <span>起点 y</span>
                      <input
                        value={stepForm.fromY}
                        type="number"
                        onChange={(event) =>
                          handleUpdateStepField('fromY', event.target.value)
                        }
                        placeholder="例如 420"
                      />
                    </label>
                  ...
                  <div className="form-grid-react compact-grid-react">
                    <label className="field-label">
                      ...
                      <input
                        ...
                        type="number"
                        onChange={(event) =>
                          handleUpdateStepField('toX', event.target.value)
                        }
                        placeholder="例如 320"
                      />
                    </label>
                    <label className="field-label">
                      ...
                      <input
                        ...
                        type="number"
                        onChange={(event) =>
                          handleUpdateStepField('toY', event.target.value)
                        }
                        placeholder="例如 620"
                      />
                    </label>
                  ...
                  <div className="form-grid-react compact-grid-react">
                    <label className="field-label">
                      <span>终点 x 偏移</span>
                      <input
                        value={stepForm.toOffsetX}
                        type="number"
                        onChange={(event) =>
                          handleUpdateStepField('toOffsetX', event.target.value)
                        }
                      />
                    </label>
                    <label className="field-label">
                      <span>终点 y 偏移</span>
                      <input
                        value={stepForm.toOffsetY}
                        type="number"
                        onChange={(event) =>
                          handleUpdateStepField('toOffsetY', event.target.value)
                        }
                      />
                    </label>
                  </div>
                  <div className="form-grid-react compact-grid-react">
                    <label className="field-label">
                      <span>按住 ms</span>
                      <input
                        value={stepForm.holdMs}
                        type="number"
                        onChange={(event) =>
                          handleUpdateStepField('holdMs', event.target.value)
                        }
                      />
                    </label>
                    <label className="field-label">
              ...
              <div className="button-row">
                ...
                <button
                  ...
                  disabled={editorDraft.steps.length === 0}
                >
                  清空动作
                </button>
              </div>
            ...
            <section className="nested-panel-react">
              ...
              <div className="form-grid-react compact-grid-react">
                <label className="field-label">
                  <span>截图 x</span>
                  <input
                    value={formatCropValue(crop.x)}
                    type="number"
                    min="0"
                    max="1"
                    step="0.01"
                    onChange={(event) =>
                      handleCropInputChange('x', event.target.value)
                    }
                  />
                </label>
                <label className="field-label">
                  <span>截图 y</span>
                  <input
                    value={formatCropValue(crop.y)}
                    type="number"
                    min="0"
                    max="1"
                    step="0.01"
                    onChange={(event) =>
                      handleCropInputChange('y', event.target.value)
                    }
                  />
                </label>
              </div>
              <div className="form-grid-react compact-grid-react">
                <label className="field-label">
                  <span>宽</span>
                  <input
                    value={formatCropValue(crop.width)}
                    type="number"
                    min="0.02"
                    max="1"
                    step="0.01"
                    onChange={(event) =>
                      handleCropInputChange('width', event.target.value)
                    }
                  />
                </label>
                <label className="field-label">
                  <span>高</span>
                  <input
                    value={formatCropValue(crop.height)}
                    type="number"
                    min="0.02"
                    max="1"
                    step="0.01"
                    onChange={(event) =>
                      handleCropInputChange('height', event.target.value)
                    }
                  />
                </label>
              </div>
              ...
              <div className="button-row">
                <button
                  type="button"
                  className="button-secondary"
                  onClick={handleCaptureAssetFromVideo}
                  disabled={!viewer.remoteStream}
                >
                  从当前画面截取模板
                </button>
              </div>
              <div className="asset-gallery-react">
                ...
                {editorDraft.images.length === 0 ? (
                  <p className="muted-text">
                    还没有图片模板，可先上传 PNG/JPG。
                  </p>
                ) : null}
              </div>
            </section>
            ...
            <section className="nested-panel-react">
              ...
              <div className="step-list">
                {editorDraft.steps.map((step, index) => (
                  ...
                  >
                    ...
                    <div className="step-details-react">
                      {buildStepDetailLines(step).map((line) => (
                ...
                {editorDraft.steps.length === 0 ? (
                  <p className="muted-text">还没有动作，可先在左侧添加步骤。</p>
                ) : null}
              </div>
              <div className="panel-header">
                <h2>JSON 预览</h2>
              ...
              <div className="button-row">
                ...
                <button
                  type="button"
                  className="button-secondary"
                  onClick={handleLoadDetailIntoEditor}
              ...
              <p
                className={
                  publishStatus.includes('失败') ? 'error-text' : 'helper-text'
                }
              >
                {publishStatus}
              </p>
      ...
      <section className="panel stack-panel execution-panel-react">
        ...
        <div className="form-grid-react compact-grid-react">
          <label className="field-label">
            <span>可用方案</span>
            <select
              value={executionSelection.packageId}
              onChange={(event) =>
                setExecutionSelection({
          ...
          <label className="field-label">
            <span>Revision</span>
            <select
              value={executionSelection.revision}
              onChange={(event) =>
                setExecutionSelection((current) => ({
                  ...current,
            ...
            >
              {selectedRevisions.length > 0 ? (
                selectedRevisions.map((entry) => (
                  <option key={entry.revision} value={String(entry.revision)}>
                    r{entry.revision} / {entry.stepCount} steps /{' '}
        ...
        <div className="button-row">
          ...
          <button
            ...
            disabled={!viewer.connected || !viewer.isExecutionRunning}
          >
            停止执行
          </button>
        </div>
        <p className="helper-text">{executionPrerequisite}</p>
        <p className={viewer.errorText ? 'error-text' : 'muted-text'}>
          {viewer.errorText || executionMessage}
        </p>
        <div className="package-list package-list-compact">
          {packages.map((entry) => (
            <button
              ...
              key={entry.packageId}
              className={
                entry.packageId === executionSelection.packageId
            ...
            >
              <div className="package-card-head">
                ...
                <span className="badge">
                  r{entry.activeRevision || entry.latestRevision || '-'}
                </span>
              </div>
              <p className="panel-copy small-copy">
                最新 {entry.latestRevision || 0} / 激活{' '}
                {entry.activeRevision || '未设置'}
              </p>
            </button>
          ))}
          {!packagesLoading && packages.length === 0 ? (
            <p className="muted-text">还没有自动化方案。</p>
          ) : null}
        </div>
      </section>
      ...
      <section className="panel stack-panel">
        ...
        <div className="room-list">
          {(health?.rooms || []).map((room) => (
            <article className="room-card" key={room.roomId}>
              <div className="room-card-head">
                ...
                <span className={room.hasPublisher ? 'badge ok' : 'badge'}>
                  {room.hasPublisher ? 'Publisher 在线' : 'Publisher 离线'}
                </span>
              </div>
          ...
          {!healthLoading && (health?.rooms || []).length === 0 ? (
            <p className="muted-text">当前没有活跃房间。</p>
          ) : null}
        </div>
      </section>
      ...
      <section className="panel stack-panel package-detail-panel">
        ...
        {detailLoading ? (
          <p className="muted-text">正在读取 revision...</p>
        ) : null}
        {detail ? (
          <>
              ...
              <section className="nested-panel-react">
                ...
                <div className="asset-gallery-react">
                  ...
                  {(detail.images || []).length === 0 ? (
                    <p className="muted-text">当前 revision 没有图片模板。</p>
                  ) : null}
                </div>
                <div className="panel-header">
                ...
                <textarea
                  className="code-preview-react code-preview-compact-react"
                  readOnly
                  value={JSON.stringify(detail.automation || {}, null, 2)}
                />
              </section>
            </div>
          </>
